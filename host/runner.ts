// J3.3 (HRN-07, HRN-11 command half, D2, TST-25 narrow, INS-02) — the ONLY file that imports
// @ai-hero/sandcastle. `kernel/` imports no plugin, no host, no sandcastle (D1); `kernel/ports/
// runner.ts` stays pure types, and this file is the one adapter that turns a `RunRequest` into a
// real agent run. M11 (D3) swaps this file for an own library, "every job file unchanged".
//
// RULING 6 — the agent has two escape routes out of the worktree, both closed here, measured:
//
...
// the check `exec()` (kernel/runtime, J3.12) runs after the pass (`git status --porcelain`) cannot
// see it. Measured with a fake `claude` running `git ls-remote origin` inside a real run:
...
export interface RunnerDeps {
   ...
   *  file someone forgets to redirect (N2 F1's lesson). Its parent directory need not exist yet;
   *  `sandcastleRunner` creates it (see below). */
...
/**
 * The pure half (layer A, plan/N3-uac.md's "what proves the real path works"): builds the
 * `AgentProvider` from a `RunRequest`, nothing else. Exported so a test can assert the exact command
 * `buildPrintCommand` renders without spending a real run — and so a test that rebuilt the provider
 * itself would not be testing its own copy of the configuration (N2 F3).
 */
